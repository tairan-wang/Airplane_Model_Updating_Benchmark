...
def make_hist(data, truth, out_base):
    ...
    for ax, (name, rng, lab) in zip(axes, PARAMS):
        ...
        ax.hist(ref, bins=bins, density=True, histtype="step",
                lw=1.8, color="#444444", ls="-", label="reference")
        P.prior_flat(ax, rng, label="Prior PDF")     # shared uniform Prior PDF
        ax.set_xlim(rng)
        ax.set_xlabel(lab)
        ax.set_ylabel("probability density")
    axes[0].legend(fontsize=8, frameon=False)
    fig.suptitle("Pooled posterior vs reference (histograms)", fontsize=12)
...
def make_kde(data, truth, out_base):
    ...
    for ax, (name, rng, lab) in zip(axes, PARAMS):
        ...
        for m in P.METHODS:
            ax.plot(xs, gaussian_kde(data[m][:, COL[name]])(xs), lw=1.8,
                    color=P.METHOD_COLOR[m], label=P.METHOD_LABEL[m])
        ax.plot(xs, gaussian_kde(truth[:, COL[name]])(xs), lw=2.0,
                color="#444444", ls="-", label="reference")
        P.prior_flat(ax, rng, label="Prior PDF")     # shared uniform Prior PDF
        ax.set_xlim(rng)
        ax.set_xlabel(lab)
        ax.set_ylabel("probability density")
    axes[0].legend(fontsize=8, frameon=False)
    fig.suptitle("Pooled posterior vs reference (KDE)", fontsize=12)
    fig.tight_layout(rect=(0, 0, 1, 0.96))
    for ext in ("png", "pdf"):
        fig.savefig(f"{out_base}.{ext}", dpi=150)
    plt.close(fig)
